...
class TwoGroupsCommonMicrostates(AbstractAnalysisStrategy):
    def __init__(self, model, alpha, interpolMicrostates, splitter: AbstractDataSplitter):
        super().__init__(model, alpha, interpolMicrostates)
        self.splitter = splitter

    def perform_analysis(self, data, path, folder_names: list[str]=['before', 'after']):
        split_data_list = self.splitter.split_data(data)
        before_split_data = split_data_list[0]
        after_split_data = split_data_list[1]
        results_list = []
        
        results_whole_data = self.model.perform_analysis(
            data=data.data,
            clustering=True,
            interpolMicrostates=self.interpolMicrostates,
        )
